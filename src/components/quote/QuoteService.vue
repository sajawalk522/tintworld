  ...
  <div class="appt-store">
    <div class="tabs">
      <div class="accordion" v-for="(service, index) in services" :key="index">
        ...
        <label class="accordion-label" :for="service.alias" @click="selectAccordion(service.alias)" :ref="getIDByService(service.alias)">
          <div class="label-content c-pointer" :class="{active: level1_selector === service.alias}">
            <div class="radio-area chk-radio">
              <b-form-group label="" >
                <b-form-radio @input="selOption" v-model="level1_selector" name="some-radios" :value="service.alias"></b-form-radio>
              </b-form-group>
            </div>
            <div class="cap-img">
  ...
  export default {
    ...
    mounted(){
      if(this.$store.state.quote.service) {
        var load_service = this.$store.state.quote.service
    ...
    methods: {
      ...
      selCheckbox(values) {
        ...
        if(values.length > 0) {
          ...
          // console.log(param)
          eventBus.$emit('isChangedQuoteValidation', true)
          this.$store.commit("setQuote", {type: 'service',data: param});
        }else{
      ...
      selOption(){
        ...
        switch(this.level1_selector) {
          case 'automotive-services':
            this.$refs.automotive[0].click()
            break;
          case 'residential-services':
            this.$refs.residential[0].click()
            break;
          case 'commercial-services':
            this.$refs.commercial[0].click()
            break;
          case 'marine-services':
            this.$refs.marine[0].click()
            break;
          default:
            this.$refs.automotive[0].click()
        }
