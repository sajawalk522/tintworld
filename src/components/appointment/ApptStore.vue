    ...
    <div class="cur-my-store" v-if="myStore">
      <div class="store-row c-pointer" :class="{active: checkActiveStore(myStore['location-id']) }"
        ...
        <div class="radio-area chk-radio">
          <b-form-group label="">
            <b-form-radio style="visibility: hidden" v-model="store_selector" name="place_radio" :value="myStore['location-id']"></b-form-radio>
          </b-form-group>
        </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      if(this.$store.state.appointment.store){
        this.selLocation(this.$store.state.appointment.store)
      }else if(this.$store.state.myStore) {
        // console.log("my store : ", this.$store.state)
        this.selLocation(this.$store.state.myStore)
        // this.$store.commit("setAppointment", {type: 'store',data: this.$store.state.myStore});
      }
    },
    computed: {
      google: gmapApi,
    ...
    methods: {
      ...
      retrieveFCID(storeNumber){
        axios.get(`${BASE_URL}/api/stores/get-store/?storeID`+storeNumber)
      ...
      selLocation(place) {
        ...
        this.$store.commit("setAppointment", {type: 'store',data: place});
      },
      checkActiveStore(location_id) {
        if(this.appointment.store) {
